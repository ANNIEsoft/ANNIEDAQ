#include "NetworkReceiveDataStream.h"

NetworkReceiveDataStream::NetworkReceiveDataStream():Tool(){}


bool NetworkReceiveDataStream::Initialise(std::string configfile, DataModel &data){
  ...
  //std::cout<<"num vme="<<m_data->numVME<<std::endl;
  Receive->connect(tmp.str().c_str());
  if(m_data->numVME>=2)Receive->connect(tmp3.str().c_str());
  if(m_data->numVME>=3)Receive->connect(tmp4.str().c_str());
  ...
  */
  ...
  //  m_data->InfoTitle="NetworkReceiveDataStreamVariables";
...
bool NetworkReceiveDataStream::Execute(){
  ...
  if(m_data->Restart==1)Finalise();
    ...
    //std::cout<<"triggered"<<std::endl;
      ...
      //for(int vme=0;vme<m_data->numVME;vme++){
      	...
      	//std::cout<<"before poll1 :"<<std::endl; 
	zmq::poll (&in[0], 1, 0);
	//std::cout<<"after poll1 :"<<std::endl; 
	//new
	...
	if (in[0].revents & ZMQ_POLLIN){
	  ...
	  if(Receive->recv(&message)){
	    ...
	    int cards;
	    memcpy ( &cards, message.data(), message.size() );
	    //    iss>>cards;
	    
	    //std::cout<<" d1 "<<cards<<std::endl;	
	    for(int i=0;i<cards;i++){
	      //std::cout<<" d2 card="<<i<<std::endl;
		...
		if (tmp.CardID>4000) std::cout<<"CardID= "<<tmp.CardID<<std::endl;
	...
	if (in2[0].revents & ZMQ_POLLIN){
	  ...
	  if(tmp2->Receive(TrigReceive)){
	    ...
	    m_data->trigdata->Save(m_data->ta);
...
bool NetworkReceiveDataStream::Finalise(){
