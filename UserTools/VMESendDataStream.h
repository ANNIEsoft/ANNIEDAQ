#ifndef VMESENDATASTREAM_H
#define VMESENDATASTREAM_H
...
class VMESendDataStream: public Tool {
 ...
 private:
  ...
  zmq::socket_t *DataRecv;
  zmq::socket_t *TrigDataRecv;

  int m_port;
  int m_trigport;
  zmq::pollitem_t out[1];
  zmq::pollitem_t out2[1];
  zmq::pollitem_t Datain[2];

  std::deque< std::vector<CardData>* > carddatavec;
  std::deque<TriggerData*> triggerdatavec;
  long datacount;
  long trigcount;
  Store statusmsg;
